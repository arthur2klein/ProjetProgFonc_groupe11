...
module Test :
  ...
  (** Cette fonction crée un test en utilisant un générateur, une fonction de réduction, une propriété et un nom. Elle renvoie le test créé
    *)
    let make_test (g : 'a Generator.t) (r : 'a Reduction.t) (name: string) (p : 'a Property.t) :'a t =
        {name = name; generator = g; reduction = r; property = p} ;;

  (** Cette fonction effectue une vérification de test en utilisant un test,   un nombre de valeurs à tester et une propriété. Si toutes les valeurs à tester satisfont les conditions. Elle retourne true 
    *)   
    let check (n : int) (test : 'a t) :bool =  
       if n <= 0 then false
          ...
          in 
          rec_check n ;;
  (** Cette fonction prend une fonction de comparaison et une liste. Si aucun élément de la liste ne satisfait la fonction, elle renvoie None, sinon elle renvoie le premier élément qui ne satisfait pas la fonction.
    *)
  ...
  (** Cette fonction prend une fonction de comparaison, un générateur et un    entier. Elle génère une valeur aléatoire à partir du générateur et renvoie la valeur qui ne satisfait pas la fonction de comparaison.    
    *)
  ...
  (** Cette fonction prend une fonction de comparaison, une fonction de réduction et une valeur. Si aucune valeur réduite de la valeur initiale ne satisfait la fonction, elle renvoie la valeur initiale, sinon elle renvoie la valeur réduite qui ne satisfait pas la fonction de comparaison. 
    *)
    ...
    ;;
  (** Cette fonction prend un nombre d'essais et un test. Elle renvoie la valeur générée qui ne satisfait pas la propriété du test.
    *)
  ...
  (** Cette fonction prend un nombre d'essais et une liste de tests. Elle renvoie une liste de tuples contenant chaque test et son résultat   
    *)
    let execute (n : int) (tests :'a t list) : ('a t * 'a option) list =
        List.map (fun test-> (test,fails_at n test)) tests
    ;;

  (** Cette fonction prend une fonction de comparaison et une liste. Elle renvoie une liste de tous les éléments de la liste qui ne satisfont pas la fonction de comparaison
    *)
    let rec find_all_not_satisfying (f : 'a -> bool) (l : 'a list) :'a list =
    List.filter (fun x -> not (f x)) l
    ;;
  (** Cette fonction prend un nombre d'essais et un test. Elle renvoie une liste de valeurs réduites qui ne satisfont pas la propriété du test 
    *)
